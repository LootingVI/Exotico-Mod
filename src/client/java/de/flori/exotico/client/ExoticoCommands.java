            ...
            dispatcher.register(literal("exotico")
                            ...
                            .then(argument("apiKey", StringArgumentType.string())
                                    .executes(context -> {
                                        ...
                                        context.getSource().sendFeedback(
                                                Text.literal("API Key updated!").formatted(Formatting.GREEN));
                                        return 1;
                                    })))
                    ...
                    .then(literal("autoscan")
                            .executes(context -> {
                                ...
                                context.getSource().sendFeedback(
                                        Text.literal("Auto-scan " + (newVal ? "enabled" : "disabled"))
                                                .formatted(newVal ? Formatting.GREEN : Formatting.RED));
                                return 1;
                            }))
                    ...
                    .then(literal("scan")
                            .then(argument("player", StringArgumentType.word())
                                    .executes(context -> {
                                        ...
                                        context.getSource().sendFeedback(
                                                Text.literal("Scanning " + target + "...")
                                                        .formatted(Formatting.YELLOW));

                                        ExoticAPI.getExoticItems(target).thenAccept(response -> {
                                            ...
                                            List<de.flori.exotico.data.ItemObject> filtered = response.exoticItemsCombined
                                                    ...
                                                    .filter(i -> i.name == null
                                                            || !i.name.toUpperCase().startsWith("FAIRY_"))
                                                    .collect(Collectors.toList());

                                            if (!filtered.isEmpty()) {
                                                context.getSource().sendFeedback(
                                                        Text.literal("Found " + filtered.size() + " exotic item(s) on "
                                                                + target + "!")
                                                                .formatted(Formatting.AQUA));

                                                filtered.forEach(item -> {
                                                    context.getSource().sendFeedback(
                                                            Text.literal("- " + item.name + " (" + item.color + ")")
                                                                    .formatted(Formatting.LIGHT_PURPLE));
                                                });
                                            } else {
                                                context.getSource().sendFeedback(
                                                        Text.literal("No exotic items found on " + target + ".")
                    ...
                    .then(literal("webhook")
                            .then(argument("url", StringArgumentType.string())
                                    .executes(context -> {
                                        ...
                                        context.getSource().sendFeedback(
                                                Text.literal("Discord Webhook updated!").formatted(Formatting.GREEN));
                                        return 1;
                                    })))
                    ...
                    .then(literal("toggle")
                            ...
                            .then(literal("sounds")
                                    .executes(context -> {
                                        boolean newVal = !ExoticoConfig.getInstance().enableSounds;
                                        ExoticoConfig.getInstance().enableSounds = newVal;
                                        ExoticoConfig.save();

                                        context.getSource().sendFeedback(
                                                Text.literal("Sounds " + (newVal ? "enabled" : "disabled"))
                                                        .formatted(newVal ? Formatting.GREEN : Formatting.RED));
                                        return 1;
                                    }))
                            ...
                            .then(literal("tooltips")
                                    .executes(context -> {
                                        boolean newVal = !ExoticoConfig.getInstance().enableTooltips;
                                        ExoticoConfig.getInstance().enableTooltips = newVal;
                                        ExoticoConfig.save();

                                        context.getSource().sendFeedback(
                                                Text.literal("Tooltips " + (newVal ? "enabled" : "disabled"))
                                                        .formatted(newVal ? Formatting.GREEN : Formatting.RED));
                                        return 1;
                                    }))
                            ...
                            .then(literal("highlight")
                                    .executes(context -> {
                                        boolean newVal = !ExoticoConfig.getInstance().enablePlayerHighlight;
                                        ExoticoConfig.getInstance().enablePlayerHighlight = newVal;
                                        ExoticoConfig.save();

                                        context.getSource().sendFeedback(
                                                Text.literal("Highlight " + (newVal ? "enabled" : "disabled"))
                                                        .formatted(newVal ? Formatting.GREEN : Formatting.RED));
                                        return 1;
                                    })))
                            ...
                            .executes(context -> {
                                MinecraftClient.getInstance().execute(() -> {
                                    MinecraftClient.getInstance().setScreen(
                                            new ExoticoSettingsScreen(MinecraftClient.getInstance().currentScreen));
                                });
                                return 1;
                            }))
                                    ...
                                    .executes(context -> {
                                        ...
                                        if (client.getNetworkHandler() != null) {
                                            for (PlayerListEntry entry : client.getNetworkHandler().getPlayerList()) {
                                                if (target.equalsIgnoreCase(entry.getProfile().name())) {
                                                    int level = PlayerScanner.extractLevel(entry);

                                                    if (level >= 0) {
                                                        context.getSource().sendFeedback(
                                                                Text.literal(target + "'s Level: ")
                                                                        .formatted(Formatting.AQUA)
                                                                        .append(Text.literal(String.valueOf(level))
                                                                                .formatted(Formatting.GOLD)));
                                                    } else {
                                                        context.getSource().sendFeedback(
                                                                Text.literal("Could not determine level for " + target
                                                                        + ". Are you on Hypixel?")
                                        ...
                                        context.getSource().sendFeedback(
                                                Text.literal("Player " + target + " not found in the current lobby.")
                                                        .formatted(Formatting.RED));
                                        return 1;
                                    })))
                    ...
                    .then(literal("clearcache")
                            .executes(context -> {
                                ...
                                context.getSource().sendFeedback(
                                        Text.literal("Scanner cache cleared!").formatted(Formatting.GREEN));
                                return 1;
                            }))
                            ...
                            .then(argument("itemId", StringArgumentType.string())
                                    .then(argument("hex", StringArgumentType.string())
                                            .executes(context -> {
                                                ...
                                                context.getSource().sendFeedback(
                                                        Text.literal(
                                                                "Checking price for " + itemId + " (#" + hex + ")...")
                                                                .formatted(Formatting.YELLOW));

                                                ExoticAPI.getPrice(itemId, hex).thenAccept(price -> {
                                                    context.getSource().sendFeedback(
                                                            Text.literal("Price for " + itemId + " (#" + hex + "): ")
                                                                    .formatted(Formatting.AQUA)
                                                                    .append(Text.literal(price.priceFormatted)
                                                                            .formatted(Formatting.GOLD)));

                                                    if (price.metadata != null) {
                                                        context.getSource().sendFeedback(
                                                                Text.literal("Confidence: " + price.metadata.confidence
                                                                        + "% | Volatility: "
                                    ...
                                    .then(argument("player", StringArgumentType.word())
                                            .then(argument("rank", StringArgumentType.word())
                                                    .executes(context -> {
                                                        ...
                                                        de.flori.exotico.api.UserAPI.setRank(target, rank);
                                                        return 1;
                                                    }))))));
        });
    }
